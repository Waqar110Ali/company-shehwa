...
import { Image, Paperclip, SendHorizonal, Smile } from "lucide-react";
...
export default function MessageInput({ onSend, loading = false }: Props) {
  ...
  async function sendText() {
    ...
    await onSend({ content: value, type: MessageType.TEXT });
    setMessage("");
  }
  ...
  async function handleFileChange(
    ...
    try {
      ...
      await onSend({
        ...
        type: uploaded.type === "IMAGE" ? MessageType.IMAGE : MessageType.FILE,
      });
    } catch (error) {
  ...
  return (
    <div className="border-t border-white/10 bg-[#111827]/95 px-3 py-3 backdrop-blur-xl sm:px-6 sm:py-4">
      <input
        hidden
        ref={fileInputRef}
        type="file"
      ...
      <div className="relative flex items-end gap-1.5 rounded-2xl border border-white/10 bg-[#1A2236] p-2 shadow-lg sm:gap-3 sm:p-3">
        {/* On very small screens, the emoji/sticker/attach icons
            shrink and lose their hover padding slightly so the row
            never needs to scroll horizontally. */}
        <button
          type="button"
          onClick={() => setShowEmojiPicker((previous) => !previous)}
          className="hidden shrink-0 rounded-xl p-2 text-zinc-400 transition hover:bg-white/10 hover:text-white xs:block"
        >
          <Smile size={20} />
        </button>
        ...
        <button
          ...
          className="hidden shrink-0 rounded-xl p-2 text-zinc-400 transition hover:bg-white/10 hover:text-white sm:block"
        >
          <Image size={20} />
        </button>
        ...
        <button
          ...
          onClick={sendText}
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50 sm:h-11 sm:w-11"
        >
          {uploading ? (
            <div className="h-5 w-5 animate-spin rounded-full border-2 border-white border-t-transparent" />
          ) : (
      ...
      <p className="mt-2 hidden text-center text-xs text-zinc-500 sm:block">
        ...
        send • <kbd className="rounded bg-white/10 px-1.5 py-0.5">Shift + Enter</kbd>{" "}
        for a new line
      </p>
    </div>
  );
