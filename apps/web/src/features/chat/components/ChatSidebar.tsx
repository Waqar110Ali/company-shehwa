...
export default function ChatSidebar({
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter((conversation) =>
      conversation.participant.fullName.toLowerCase().includes(keyword),
    );
  }, [conversations, search]);

  async function handleCreate(employeeId: string) {
    try {
  ...
  return (
    <>
      <aside className="flex h-full min-h-0 flex-col bg-[#0F172A]">
        <div className="shrink-0 border-b border-white/10 bg-[#0F172A] p-4 md:p-5">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-2 md:p-3">
          {filtered.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center px-6 text-center">
              <MessageCirclePlus size={48} className="mb-4 text-zinc-600" />
