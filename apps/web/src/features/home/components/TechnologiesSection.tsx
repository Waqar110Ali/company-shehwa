// features/home/components/TechnologiesSection.tsx
import AuroraBackground from "@/components/effects/AuroraBackground";
import Container from "@/components/common/Container";
import Section from "@/components/common/Section";
import FadeUp from "@/components/motion/FadeUp";
import GradientText from "@/components/premium/GradientText";

import TechnologyCategory from "./TechnologyCategory";

import { useSectionContent } from "../hooks/useSectionContent";
import { technologyCategories } from "../constants/technologies";

export default function TechnologiesSection() {
  const categories = useSectionContent("technologyCategories", technologyCategories);

  return (
    <AuroraBackground>
      <Section className="relative py-32">
        <Container>
          <FadeUp>
            <div className="mx-auto max-w-4xl text-center">
              <span className="rounded-full border border-cyan-400/20 bg-cyan-500/10 px-5 py-2 text-sm font-medium text-cyan-300 backdrop-blur-xl">
                Technology Stack
              </span>

              <h2 className="mt-8 text-5xl font-black text-white lg:text-6xl">
                Powered by
                <GradientText> Modern Technologies</GradientText>
              </h2>

              <p className="mx-auto mt-8 max-w-3xl text-lg leading-8 text-slate-300">
                We use enterprise-grade technologies to build secure, scalable,
                AI-powered software solutions that perform reliably at every stage of growth.
              </p>
            </div>
          </FadeUp>

          <div className="mt-24 grid gap-8 md:grid-cols-2 xl:grid-cols-3">
            {categories.map((category) => (
              <TechnologyCategory key={category.title} title={category.title} items={category.items} />
            ))}
          </div>
        </Container>
      </Section>
    </AuroraBackground>
  );
}