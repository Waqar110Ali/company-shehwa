// features/home/components/CompanyStatistics.tsx
...
export default function CompanyStatistics() {
  const items = useSectionContent("statistics", statistics);
      ...
      <Section className="relative py-32">
        <Container>
          <FadeUp>
            <div className="mx-auto max-w-4xl text-center">
              ...
              <h2 className="mt-8 text-5xl font-black text-white lg:text-6xl">
                Numbers That
                <GradientText> Speak for Themselves</GradientText>
              </h2>

              <p className="mx-auto mt-8 max-w-3xl text-lg leading-8 text-slate-300">
                We measure our success by delivering real value, building long-term
                partnerships, and helping businesses grow through modern technology.
              </p>
            </div>
          </FadeUp>
          ...
          <div className="mt-24 grid gap-8 sm:grid-cols-2 xl:grid-cols-4">
            {items.map((item) => (
              <StatisticCard key={item.title} title={item.title} value={item.value} icon={getIcon(item.icon)} />
            ))}
          </div>
        </Container>
      </Section>
